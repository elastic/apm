...
Therefore, the agent should send its final intake request at the end of the function invocation with the query param `flushed=true`. In case there is no more data to send at the end of the function invocation, the agent should send an empty intake request with this query param.
