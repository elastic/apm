...
Note that `faas.*` fields *are not* nested under the context property [in the intake api](https://github.com/elastic/apm-data/blob/main/input/elasticapm/docs/spec/v2/transaction.json)! `faas` is a top-level key on the transaction.
