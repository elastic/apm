...
* *host-id*: The [`host.id` resource attribute](https://opentelemetry.io/docs/specs/semconv/attributes-registry/host/) used for the profiling data by this profiling host agent. If an APM-agent is already sending a `host.id` with it's transactions and spans it SHOULD print a warning if the `host.id` is different and otherwise ignore the value received by the host agent. If an agent does not collect the `host.id` by default, it MUST start sending the `host.id` after receiving it from the profiler host agent to ensure certain correlation features (e.g. cost and CO2 consumption) work correctly.
