...
The `agent.events.total` metric MUST have the label `event_type`. The possible values for `event_type` are the lowercase names of the events according to the [spec](https://github.com/elastic/apm-data/tree/main/input/elasticapm/docs/spec/v2) (e.g. `transaction`, `metricset`) or the value `undefined`.
